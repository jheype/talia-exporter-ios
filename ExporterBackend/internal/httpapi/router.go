...
func NewRouter(
	...
	api.HandleFunc("GET /api/v1/exporter/groups/history-sync", handler.historySyncStatus)
	api.HandleFunc("POST /api/v1/exporter/groups/history-sync", handler.retryHistorySync)
	api.HandleFunc("GET /api/v1/exporter/messages", handler.messages)
	api.HandleFunc("POST /api/v1/exporter/messages/ingestion", handler.markMessagesQueuedForIngestion)
